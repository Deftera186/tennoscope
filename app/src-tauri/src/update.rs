...
pub fn classify(
    ...
    if os == "linux" {
        if appimage.is_some() {
            ...
            // (/tmp/.mount_*). Without that corroboration a stray $APPIMAGE on
            // a system/dev binary must not promote it to Appimage.
...
fn current_classification() -> (InstallKind, bool) {
    ...
    let writable = appimage
        .as_ref()
        .is_some_and(|p| std::fs::metadata(p).is_ok_and(|m| !m.permissions().readonly()));
    // Machine-wide roots for the prefix comparison. Empty everywhere else.
...
mod tests {
    ...
    fn appimage_needs_mount_corroboration() {
        ...
        let (kind, up) = classify(
            Path::new("/tmp/.mount_foo/usr/bin/tennoscope"),
